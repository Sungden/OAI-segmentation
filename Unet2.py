...
validation_X=X_test[0:4800]
validation_Y=Y_test[0:4800]
    ...
    while True:
        ...
        for i in range(len(X_batches)):
            ...
            for m in range(y.shape[0]):                      
              if np.count_nonzero(y[m,:,:])>0:     #FC at  least 500 pixels
                rows.append(m)
            yield x[rows,:,:],y[rows,:,:]
...
log_filename = 'outputs2/' + 'TC_new_model_train2.csv' 
#Callback that streams epoch results to a csv file.

csv_log = callbacks.CSVLogger(log_filename, separator=',', append=True)
early_stopping = callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=50, verbose=0, mode='min')
checkpoint_filepath = 'outputs2/' + 'TC_weights2.h5'
...
model_name = 'outputs2/' +  'TC_new_model_last.h5'
