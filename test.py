...
model.load_weights("/data/ydeng1/OAI/Unet_2D/outputs2/weights2.h5")
...
for X,Y in zip(test_img,test_gt):
    ...
    '''  
    plt.subplot(2,4,1)
    plt.imshow(y_pred[0,:,:,1],'gray')
    plt.subplot(2,4,2)
    plt.imshow(y_pred[0,:,:,2],'gray')
    plt.subplot(2,4,3)
    plt.imshow(y_pred[0,:,:,3],'gray')
    plt.subplot(2,4,4)
    plt.imshow(y_pred[0,:,:,4],'gray')
    plt.subplot(2,4,5)
    ...
    dice_FB=computeDice(FB,FB_gt)
    DSC_FB.append(dice_FB)
    print(dice_FB,'$$$$$$$$$$$')
    
    dice_FC=computeDice(FC,FC_gt)
    DSC_FC.append(dice_FC)
    
    dice_TB=computeDice(TB,TB_gt)
    DSC_TB.append(dice_TB)
    
    dice_TC=computeDice(TC,TC_gt)
    DSC_TC.append(dice_TC)
    f = open("result_new.txt", 'a')
    a=[dice_FB,dice_FC,dice_TB,dice_TC]
    f.write(str(a))
    f.write("\n")
    '''
    img = nib.Nifti1Image(y_predi, np.eye(4))
    output_name=X[0:7]+'.nii.gz'
    print(output_name)
    
    nib.save(img, os.path.join(output,output_name))
    '''
time2=time.time()
print(len(DSC_FB))

print('mean dice of FB is:',np.mean(DSC_FB))
print('max dice of FB is:',np.max(DSC_FB))
print('min dice of FB is:',np.min(DSC_FB)) 

print('mean dice of FC is:',np.mean(DSC_FC))
print('max dice of FC is:',np.max(DSC_FC))
print('min dice of FC is:',np.min(DSC_FC)) 

print('mean dice of TB is:',np.mean(DSC_TB))
print('max dice of TB is:',np.max(DSC_TB))
print('min dice of TB is:',np.min(DSC_TB)) 
